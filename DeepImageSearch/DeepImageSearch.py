...
# LoadData() class
# fromFolder: returns a list of image folders

# Index() class
# init: creates metadata folder, featureExtractor object
# Start: calls start_feature_extraction and uses that data in start_indexing
# start_feature_extraction: calls get_feature of FeatureExtractor class and returns map of images to features
# start_indexing: stores AnnoyIndex in euclidean metric which is a vector of f dimensions
#                 the forest is built with 100 trees

# featureExtractor() class
# uses VGG-16 model that returns features from fully connected layer
# The model is extended by adding Dense layers on top and running the whole thing end to end on the input data.
# This will allow using data augmentation because every input image goes through the convolutional base
# every time it is seen by the model.
# get_feature: for each image, calls extract method and returns a list of all features
# extract: extracts the features of image and returns a normalized vector

# SearchImage() class
# getSimilarImages: gets query vector for input image from get_query_vector
#                   return similar images from search_by_vector
# get_query_vector: creates featureExtractor object and returns the result of its extract method
# search_by_vector: returns n nearest neighbours from the index where n is the number of recommendations
...
class LoadData:
    ...
    def from_folder(self,folder_list:list): # Enter the Single Folder Path/List of the Folders
        ...
        return image_path # Returning list of image folder paths
...
class SearchImage:
    ...
    def search_by_vector(self,v,n:int):
        ...
        index_list = u.get_nns_by_vector(self.v, self.n) # will find the n nearest neighbors
    ...
    def get_query_vector(self,image_path:str):
        ...
        return query_vector
    def plot_similar_images(self,image_path:str,number_of_images:int):
        self.image_path = image_path
